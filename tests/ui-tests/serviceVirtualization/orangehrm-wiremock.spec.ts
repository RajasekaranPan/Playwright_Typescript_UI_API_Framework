...
test.describe('Service Virualization => Playwright Intercept + WireMock',{tag: ['@Regression'],
...
() => {
 ...
 test('WireMock test', {tag: ['@EndToEnd','@network'],
    annotation: [{
        type: "Test Case Link",
    ...
    await page.route(
        ...
        async route => {
            ...
            const wireMockUrl =
                `http://localhost:8080${new URL(request.url()).pathname}`;
            ...
            console.log(
                'WireMock response status:',
                wireMockResponse.status()
            );
