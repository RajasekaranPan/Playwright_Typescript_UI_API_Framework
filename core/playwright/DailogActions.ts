...
await this.actions.dialog.accept(
   ...
   */
  async accept(action: () => Promise<void>): Promise<void> {
   ...
   */
  async dismiss(action: () => Promise<void>): Promise<void> {
    const dialogPromise = this.page.waitForEvent('dialog');
   ...
   */
  async acceptPrompt(action: () => Promise<void>, value: string): Promise<void> {
    const dialogPromise = this.page.waitForEvent('dialog');
