...
import { EmployeeData, MaritalStatus } from './types';
...
export class EmployeeDataProvider {
  private static readEmployees(): EmployeeData[] {
    ...
    if (!worksheet) {
      throw new Error(`Excel sheet '${DataSheet.employee}' not found`);
    }

    return XLSX.utils.sheet_to_json<EmployeeData>(worksheet, {
      defval: '',
    });
  }
